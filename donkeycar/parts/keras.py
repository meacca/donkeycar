...
class KerasLinearNew(KerasPilot):
    '''
    The KerasLinear pilot uses one neuron to output a continous value via the 
    Keras Dense layer with linear activation. One each for steering and throttle.
    The output is not bounded.
    '''
    def __init__(self, num_outputs=2, input_shape=(120, 160, 3), roi_crop=(0, 0), *args, **kwargs):
        super(KerasLinearNew, self).__init__(*args, **kwargs)
        self.model = default_n_linear_new(num_outputs, input_shape, roi_crop)
        self.compile()
...
def default_n_linear_new(num_outputs, input_shape=(120, 160, 3), roi_crop=(0, 0)):
